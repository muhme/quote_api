...
const RESPONSES = {
  '200': {
    description:
      'OK – a random quote retrieved successfully. \
  ...
  '400': {
    ...
    content: {
      ...
      'text/html': {
        example:
          '<html> ... <h1>BadRequestError</h1> <h2><em>400</em> Parameter &#39;userId&#39; must be a positive integer.</h2> ...',
      },
    },
  },
  '404': {
      ...
      'application/json': {
        example: {
          error: {
            ...
            name: 'NotFoundError',
            message:
              'No quote found for given parameters: language=es (Spanish), userId=42 (rezitant), categoryId=42 (Carpintero), authorId=42 (Jean Paul)!',
          },
        },
      },
      'text/html': {
        example:
          '<html> ... <h1>NotFoundError</h1> <h2><em>404</em> No quote found for given parameters: userId=42 (rezitant), categoryId=21 (Success), authorId=42 (Jean Paul).</h2> ...',
      },
    },
  },
  '500': {
...
const RESPONSES_HTML = {
  ...
  '400': {
    ...
    content: {
      'text/html': {
        example:
          '<html> ... <h1>BadRequestError</h1> <h2><em>400</em> \
  ...
  '404': {
    ...
    content: {
      'text/html': {
        example:
          '<html> ... <h1>NotFoundError</h1> <h2><em>404</em> No quote \
      ...
      'application/json': {
        example: {
          error: {
            ...
            name: 'NotFoundError',
            message:
              'No quote found for given parameters: language=es (Spanish), \
...
export class QuotationController {
  ...
  @get('/v1/quote', {
    ...
    summary:
      'Get one random quote with source, author and links in JSON format.',
    description:
      'Get one random quote with source, author and links in JSON \
  ...
  async getQuotationJson(
    ...
    if (!quote || quote.length !== 1) {
      throw new HttpErrors.NotFound(
        `Could not find a quote for given parameters.`,
      );
    }
  ...
  @get('/v1/quote_html', {
    ...
    summary:
      'Get a random quote with source, author and links in HTML format with optional style.',
    description:
      'Get a random quote with source, author and links in HTML \
  ...
  async getQuotations(
    ...
    const result = await this.mapQuoteToHtml(
      quote[0],
      style,
      contentOnly,
      target,
  ...
  private async mapQuoteToRandomQuote(quote: Quotation): Promise<RandomQuote> {
    ...
    const author: Author | undefined = await this.authorsRepository.findAuthor({
      language: quote.language,
  ...
  private validateParameters(
    ...
    if (userId && userId < 0) {
      throw new HttpErrors.BadRequest(
        "Parameter 'userId' must be a positive integer.",
      );
    }
    if (authorId && authorId < 0) {
      throw new HttpErrors.BadRequest(
        "Parameter 'authorId' must be a positive integer.",
      );
    }
    if (categoryId && categoryId < 0) {
      throw new HttpErrors.BadRequest(
        "Parameter 'categoryId' must be a positive integer.",
      );
    }
  }
  ...
  private async throwNotFoundWithGivenParameters(filter: QuoteFilter) {
    ...
    if (filter.language !== undefined) {
      givenParams.push(
        `language=${filter.language} (${LANGUAGES[filter.language]})`,
      );
    }
    ...
    if (filter.categoryId !== undefined) {
      const categoryName = await this.categoriesRepository.categoryName(
        filter.categoryId,
        filter.language,
      );
    ...
    if (filter.authorId !== undefined) {
      const authorName = await this.authorsRepository.authorName(
        filter.authorId,
        filter.language,
      );
    ...
    throw new HttpErrors.NotFound(
      `No quote found for given parameters: ${givenParams.join(', ')}.`,
    );
  }
  ...
  //   </div>
  // </div>
  private async divContainers(
    quote: Quotation,
    target: string | undefined,
  ): Promise<string> {
    const link =
      ZITAT_SERVICE_DE_URL + `/${quote.language}/quotations/${quote.id}`;
    ...
    if (quote.source || quote.authorId !== 0) {
      ...
      if (quote.authorId !== 0) {
        ...
        const author: Author | undefined =
          await this.authorsRepository.findAuthor({
            language: quote.language,
            authorId: quote.authorId ?? 0,
          });
