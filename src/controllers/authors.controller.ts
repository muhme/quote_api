...
const AUTHORS_RESPONSES = {
  '200': {
    description:
      'OK – the list of author entries retrieved successfully. \
          ...
          authors: [
            {
              ...
              firstname: 'Woody',
              description:
                'US-American director, actor, comedian, writer and musician (born 1935)',
  ...
  '400': {
    ...
    content: {
      ...
      'text/html': {
        example:
          '<html> ... <h1>BadRequestError</h1> <h2><em>400</em> Parameter &#39;authorId&#39; must be a positive integer.</h2> ...',
      },
    },
  },
  '404': {
      ...
      'application/json': {
        example: {
          error: {
            ...
            name: 'NotFoundError',
            message:
              "No authors found for given parameters (language: 'en', page: 1, size: 100, lastname: 'XXX').",
          },
        },
      },
      'text/html': {
        example:
          '<html> ... <h1>NotFoundError</h1> <h2><em>404</em> No authors found for given parameters (language: &#39;en&#39;, page: &#39;1&#39;, size: &#39;100&#39;, lastname: &#39;XXX&#39;).</h2> ...',
      },
    },
  },
  '500': {
...
const AUTHOR_RESPONSES = {
  '200': {
    description:
      'OK – the author entry was retrieved successfully. \
...
export class AuthorsController {
  ...
  @get('/v1/authors', {
    ...
    description:
      'Get paged list of authors. List can be restricted with \
  ...
  async getAuthors(
    @param.query.string('language', {
      description:
        'The `language` for the author entries. See `/v1/languages` for available language codes.',
      schema: {
        type: 'string',
        default: 'en',
      },
    ...
    @param.query.string('lfd', {
      description:
        'The beginning of the authors `lastname,firstname,description` \
    ...
    if (filter.lfd) {
      [filter.lastname, filter.firstname, filter.description] =
        filter.lfd.split(',', 3);
      delete filter.lfd;
    }
    ...
    const authorsPaged: AuthorsPaged =
      await this.authorsRepository.findAuthors(filter);

    if (authorsPaged.authors.length === 0) {
      throw new HttpErrors.NotFound(
        `No authors found for given parameters (${myStringify(filter)}).`,
      );
    }
  ...
  @get('/v1/author', {
    ...
    description:
      'Get one author entry for given `authorId` in given `language`. \
  ...
  async getAuthor(
    @param.query.string('language', {
      description:
        'The `language` for the author entry. See `/v1/languages` for all available languages.',
      schema: {
        type: 'string',
        default: 'en',
      },
    ...
    @param.query.integer('authorId', {
      description:
        'Authors ID. For a list of all author entries with their IDs see `/v1/authors`.',
      schema: {
        type: 'integer',
        default: 1,
      },
    ...
    const author: Author | undefined =
      await this.authorsRepository.findAuthor(filter);

    if (!author) {
      throw new HttpErrors.NotFound(
        `No author entry found for ID #${filter.authorId}!`,
      );
    }
