...
const RESPONSES = {
  '200': {
    description:
      'OK – the login names and their IDs are retrieved successfully. The result is sorted by login names.',
    content: {
      'application/json': {
        example: {
          paging: {
  ...
  '404': {
    ...
    content: {
      ...
      'text/html': {
        example:
          '<html> ... <h1>NotFoundError</h1> <h2><em>404</em> No user entries found for given parameters.</h2> ...',
      },
    },
  },
  '500': {
...
export class UsersController {
  ...
  @get('/v1/users', {
    ...
    description:
      'Get users login names and their IDs. Only users who have \
  ...
  async getUsers(
    @param.query.integer('page', {
      description:
        'The response is made page by page, the optional parameter \
    ...
    const usersPaged =
      await this.usersRepository.findUsersWithQuotations(filter);

    if (usersPaged.users.length === 0) {
      throw new HttpErrors.NotFound(
        'No user entries found for given parameters.',
      );
    }
